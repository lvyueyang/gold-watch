...
export async function collectAndNotify(env?: CloudflareEnv, ctx?: ExecutionContext): Promise<{ tick: PriceTick | null; matched: number }> {
  ...
  if (kv) {
    await kv.put(`quote:${tick.instrumentId}`, JSON.stringify(tick), { expirationTtl: 86400 });
  }
